...
uint8_t readRegBuff(uint8_t reg, uint8_t* buf, uint8_t len)
{
  ...
  csn(LOW);
  status = spiSendData( R_REGISTER | ( REGISTER_MASK & reg ) );
  while ( len-- )
...
{
  csn(LOW);
  spiSendData( R_REGISTER | ( REGISTER_MASK & reg ) );
...
void setChannel(uint8_t channel)
{
  ...
  writeRegChar(RF_CH,min(channel,max_channel));
}
...
void begin(void)
{
  ...
  writeRegChar(SETUP_RETR,(B0100 << ARD) | (B1111 << ARC));
  ...
  writeRegChar(DYNPD,0);

  // Reset current status
  // Notice reset and flush is the last thing we do
  writeRegChar(STATUS,_BV(RX_DR) | _BV(TX_DS) | _BV(MAX_RT) );
...
{
  writeRegChar(CONFIG, readRegChar(CONFIG) | _BV(PWR_UP) | _BV(PRIM_RX));
  writeRegChar(STATUS, _BV(RX_DR) | _BV(TX_DS) | _BV(MAX_RT) );
...
void powerDown(void)
{
  writeRegChar(CONFIG,readRegChar(CONFIG) & ~_BV(PWR_UP));
}
...
bool write( const void* buf, uint8_t len )
{
  ...
  do
  {
    status = readRegBuff(OBSERVE_TX,&observe_tx,1);
...
uint8_t getDynamicPayloadSize(void)
{
  ...
  csn(LOW);
  spiSendData( R_RX_PL_WID );
  result = spiSendData(0xff);
  csn(HIGH);
...
bool available(uint8_t* pipe_num)
{
  ...
  if (result)
  {
    ...
    writeRegChar(STATUS,_BV(RX_DR) );

    // Handle ack payload receipt
    if ( status & _BV(TX_DS) )
    {
      writeRegChar(STATUS,_BV(TX_DS));
    }
  }
...
bool read( void* buf, uint8_t len )
{
  ...
  return readRegChar(FIFO_STATUS) & _BV(RX_EMPTY);
}
...
void whatHappened(bool& tx_ok,bool& tx_fail,bool& rx_ready)
{
  ...
  uint8_t status = writeRegChar(STATUS,_BV(RX_DR) | _BV(TX_DS) | _BV(MAX_RT) );
...
void openReadingPipe(uint8_t child, uint64_t address)
{
  ...
  if (child <= 6)
  {
    ...
    writeRegChar(EN_RXADDR,readRegChar(EN_RXADDR) | _BV(pgm_read_byte(&child_pipe_enable[child])));
  }
}
...
void enableDynamicPayloads(void)
{
  ...
  writeRegChar(FEATURE,readRegChar(FEATURE) | _BV(EN_DPL) );

  // If it didn't work, the features are not enabled
  if ( ! readRegChar(FEATURE) )
  {
  ...
  writeRegChar(DYNPD,readRegChar(DYNPD) | _BV(DPL_P5) | _BV(DPL_P4) | _BV(DPL_P3) | _BV(DPL_P2) | _BV(DPL_P1) | _BV(DPL_P0));
...
void enableAckPayload(void)
{
  ...
  writeRegChar(FEATURE,readRegChar(FEATURE) | _BV(EN_ACK_PAY) | _BV(EN_DPL) );
  ...
  writeRegChar(DYNPD,readRegChar(DYNPD) | _BV(DPL_P1) | _BV(DPL_P0));
}
...
{
  if ( enable )
    writeRegChar(EN_AA, B111111);
  else
    writeRegChar(EN_AA, 0);
}
...
void setAutoAck( uint8_t pipe, bool enable )
{
  if ( pipe <= 6 )
  {
    uint8_t en_aa = readRegChar( EN_AA ) ;
    if( enable )
    {
      en_aa |= _BV(pipe) ;
    }
...
{
  uint8_t disable = readRegChar(CONFIG) & ~_BV(EN_CRC) ;
  writeRegChar( CONFIG, disable ) ;
}

/****************************************************************************/
void setRetries(uint8_t delay, uint8_t count)
{
 writeRegChar(SETUP_RETR,(delay&0xf)<<ARD | (count&0xf)<<ARC);
}
